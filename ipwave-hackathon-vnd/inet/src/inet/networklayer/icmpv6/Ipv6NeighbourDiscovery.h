...
//    typedef Ipv6NeighbourCache::MultihopDadEntry MultihopDadEntry;    // for convenience
    ...
    struct tentativeRouter
    {
        Ipv6Address routerAddr;
        Ipv6Address relayAddr;
        int hopDistance;
        MacAddress relayLinkLayerAddr;
        Ipv6NdPrefixInformation prefixInfo;
        tentativeRouter (Ipv6Address router, Ipv6Address relay, int hop, MacAddress relayMac,Ipv6NdPrefixInformation prefix) :
            routerAddr(router), relayAddr(relay), hopDistance(hop),relayLinkLayerAddr(relayMac), prefixInfo(prefix) {}
    };


    struct sortByHopDistance
    {
        inline bool operator() (const tentativeRouter& struct1, const tentativeRouter& struct2)
        {
            return (struct1.hopDistance < struct2.hopDistance);
        }
    };
  ...
  public:
    //Change neighbourCache and rt6 from protected to public
    ...
    // signals
     ...
     static simsignal_t startDadSignalTime;
     static simsignal_t startDadSignalId;
     static simsignal_t responseDadSignalTime;
     static simsignal_t receivedDadSignalTime;
     static simsignal_t finishDadSignalTime;
     static simsignal_t responseDadSignalId;
     static simsignal_t receivedDadSignalId;
     static simsignal_t finishDadSignalId;

    static simsignal_t numDadMessageForwardedSignal;
    static simsignal_t numSentSignal;
    static simsignal_t numReceivedSignal;


    long numReceived = 0;
    long numSent = 0;
    long numDadMessageForwarded = 0;

   // simtime_t startDad;
   // simtime_t responseDad;
   // simtime_t receivedDad;
   // simtime_t finishDad;
    ...
    std::vector <tentativeRouter> tentativeRouterList;
  ...
  protected:
    ...
    multihopDadList multihopdadList;

    int hopDistance = 1;
    int neighborHopDistance = 10;
    Ipv6NdPrefixInformation tentativePrefixInfo;
  ...
  protected:
    ...
    virtual void delayProcessNaFromNeighbor(cMessage *msg);
    virtual void processNdTimeout(cMessage *msg);
    virtual void msgTriggerForMultihopDAD(cMessage *msg);

    virtual void processNsAtMA(Packet *packet, const Ipv6NeighbourSolicitation *ns, InterfaceEntry *ie);
    virtual void processNaFromMA(Packet *packet, const Ipv6NeighbourAdvertisement *na);
    ...
    virtual void processMultihopDadTimeout(cMessage *msg);
    /**
