...
//    enum ReachabilityState { INCOMPLETE, REACHABLE, STALE, DELAY, PROBE };
    ...
    //XIANG ZHONG 19.06.10
    struct neighborRoute
    {
        const Key *nceKey = nullptr;  //addr, interfaceid
        MacAddress macAddr;
        Ipv6Address relayAddr; //next-hop address
        int hopDistance;
//        neighborRoute (Ipv6Address addr, MacAddress mac, Ipv6Address relay, int hop) :
//            neighborAddr(addr), macAddr(mac), relayAddr(relay), hopDistance(hop) {}
    };
    ...
    typedef std::map <Key, Neighbour> NeighborRouteMap;
  ...
  protected:
    ...
    //XIANG ZHONG
    //integrated DAD table at MA
    MultihopDadMap multihopDadMap;

    //XIANG ZHONG 19.06.10
    //the new routing table including the information in NCEs
    NeighborRouteMap neighborRouteMap;
  ...
  public:
    ...
    virtual Neighbour *addNeighborRoute (const Ipv6Address &addr, int interfaceID, MacAddress macAddress,
            const Ipv6Address& relayAddr, int hopDistance);

    virtual Neighbour *lookupNeighborRoute(const Ipv6Address& addr, int interfaceID);
    //
    ...
    //XIANG ZHONG
    virtual Neighbour *addTentativeNeighbour(const Ipv6Address& globalAddr, const Ipv6Address& linkLocalAddr, int interfaceID, MacAddress macAddress, short registLifetime);
    virtual Neighbour *addRelayNeighbour(const Ipv6Address& globalAddr, const Ipv6Address& linkLocalAddr, int interfaceID, MacAddress macAddress, short registLifetime, const Ipv6Address& relayAddr);
    //Multihop DAD Table
    ...
    virtual Neighbour *addRouterWithGlobalAddr(const Ipv6Address& addr, int interfaceID,
                MacAddress macAddress, simtime_t expiryTime, const Ipv6Address& globalAddr,  bool isHomeAgent = false);    // added HA flag, 3.9.07 - CB
