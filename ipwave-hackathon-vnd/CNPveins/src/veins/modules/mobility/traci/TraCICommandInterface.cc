...
void TraCICommandInterface::Vehicle::setPosition(double x, double y) {
    uint8_t variableId = VAR_POSITION;
    uint8_t variableType = POSITION_2D;
    TraCIBuffer buf = traci->connection.query(CMD_SET_VEHICLE_VARIABLE, TraCIBuffer() << variableId << nodeId << variableType <<x <<y);
    ASSERT(buf.eof());
}
TraCICoord TraCICommandInterface::Vehicle::ReturnSumoCurrentposition(Coord CurPos){
    return traci->connection.omnet2traci(CurPos);
}
std::string TraCICommandInterface::Vehicle::getLeadeID(std::string curID, double range){

}
double TraCICommandInterface::Vehicle::getLeaderdst(std::string curID, double range){
    return traci->genericGetDouble(CMD_GET_VEHICLE_VARIABLE, curID, VAR_LEADER, RESPONSE_GET_VEHICLE_VARIABLE);
}
