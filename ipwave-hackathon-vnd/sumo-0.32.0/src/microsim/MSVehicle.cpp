...
MSVehicle::Influencer::influenceChangeDecision(const SUMOTime currentTime, const MSEdge& currentEdge, const int currentLaneIndex, int state) {
    ...
    if ((state & LCA_WANTS_LANECHANGE_OR_STAY) != 0) {
        ...
        if ((state & LCA_TRACI) != 0 && myLatDist != 0) {
            ...
            std::cout <<" state of Changing Lane 1 = "<<state<<std::endl;
            return state;
        } else if ((state & LCA_STRATEGIC) != 0) {
            mode = myStrategicLC;
            std::cout <<" state of Changing Lane myStrategicLC = "<<myStrategicLC<<std::endl;
        } else if ((state & LCA_COOPERATIVE) != 0) {
            mode = myCooperativeLC;
            std::cout <<" state of Changing Lane myCooperativeLC = "<<myCooperativeLC<<std::endl;
        } else if ((state & LCA_SPEEDGAIN) != 0) {
            mode = mySpeedGainLC;
            std::cout <<" state of Changing Lane mySpeedGainLC = "<<mySpeedGainLC<<std::endl;
        } else if ((state & LCA_KEEPRIGHT) != 0) {
            mode = myRightDriveLC;
            std::cout <<" state of Changing Lane myRightDriveLC = "<<myRightDriveLC<<std::endl;
        } else if ((state & LCA_SUBLANE) != 0) {
            mode = mySublaneLC;
            std::cout <<" state of Changing Lane mySublaneLC = "<<mySublaneLC<<std::endl;
       } else if ((state & LCA_TRACI) != 0) {
            mode = LC_NEVER;
            std::cout <<" state of Changing Lane LC_NEVER = "<<LC_NEVER<<std::endl;
        } else {
...
/*// BAM
double MSVehicle:: mySpeedAccess(){
    return getSpeed();
}
// BAM*/
