...
// debug constants
// ===========================================================================
  // #define DEBUG_SIMSTEP      //Uncommented by BAM
...
MSNet::simulationStep() {
   ...
   /* if (MSGlobals::gCheck4Accidents) {
        ...
        std::cout << "detectCollisions() called in simulationStep()" <<std::endl;
    }*/ //BAM Comment
    ...
    if (MSGlobals::gUseMesoSim) {
       ...
       /* if (MSGlobals::gCheck4Accidents) {
            ...
            std::cout << "detectCollisions() called in simulationStep() 2" <<std::endl;
        }*/ //BAM Comment

        // vehicles may change lanes
        std::cout <<"Step !!__________________________________!!Step" <<std::endl;
        myEdges->changeLanes(myStep);

       /* if (MSGlobals::gCheck4Accidents) {
            myEdges->detectCollisions(myStep, STAGE_LANECHANGE);
            std::cout << " ---- detectCollisions() called in simulationStep() 3 ----" <<std::endl;
        }*/ //BAM Comment
    }
    loadRoutes();
