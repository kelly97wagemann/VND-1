...
#define KEEP_RIGHT_HEADWAY (double)2.0 //new add

#define URGENCY (double)2.0

#define ROUNDABOUT_DIST_BONUS (double)100.0

#define KEEP_RIGHT_TIME (double)500.0 //BAM prev 5.0 the number of seconds after which a vehicle should move to the right lane
...
#define TURN_LANE_DIST (double)2000.0 //BAM prev 200.0 the distance at which a lane leading elsewhere is considered to be a turn-lane that must be avoided
...
// ===========================================================================
MSLCM_BAM2019::MSLCM_BAM2019(MSVehicle& v) :
    MSAbstractLaneChangeModel(v, LCM_BAM2019),
...
MSLCM_BAM2019::~MSLCM_BAM2019() {
    changed();
}


void
MSLCM_BAM2019::initDerivedParameters() {
...
bool
MSLCM_BAM2019::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_BAM2019::wantsChangeSublane(
    int laneOffset,
    ...
    std::cout <<"--------> MSLCM_BAM2019::wantsChangeSublane() Called!!" <<std::endl;
...
void
MSLCM_BAM2019::setOwnState(const int state) {
...
void
MSLCM_BAM2019::updateSafeLatDist(const double travelledLatDist) {
...
double
MSLCM_BAM2019::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    gDebugFlag2 = DEBUG_COND;
...
double
MSLCM_BAM2019::_patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    if (wanted <= 0) {
        return wanted;
    }
...
void*
MSLCM_BAM2019::inform(void* info, MSVehicle* sender) {
...
void
MSLCM_BAM2019::msg(const CLeaderDist& cld, double speed, int state) {
    assert(cld.first != 0);
...
double
MSLCM_BAM2019::informLeader(int blocked,
                           int dir,
...
void
MSLCM_BAM2019::informFollower(int blocked,
                             int dir,
...
double
MSLCM_BAM2019::informLeaders(int blocked, int dir,
...
void
MSLCM_BAM2019::informFollowers(int blocked, int dir,
                              const std::vector<CLeaderDist>& blockers,
                              double remainingSeconds,
                              double plannedSpeed) {
    for (std::vector<CLeaderDist>::const_iterator it = blockers.begin(); it != blockers.end(); ++it) {
    ...
    if (myExpectedSublaneSpeeds.size() != myVehicle.getLane()->getEdge().getSubLaneSides().size()) {
        ...
        if (myExpectedSublaneSpeeds.size() > 0) {
            ...
            if (myLastEdge->getSubLaneSides().size() == myExpectedSublaneSpeeds.size()) {
                ...
                std::cout << " in prepareStep(), subLaneShift = " <<subLaneShift <<std::endl;
...
int
MSLCM_BAM2019::computeSublaneShift(const MSEdge* prevEdge, const MSEdge* curEdge) {
    // find the first lane that targets the new edge
    // WRITE_MESSAGE("Sublane Shift  MSLCM_BAM2019::computeSublaneShift()");
    std::cout <<"Sublane Shift  MSLCM_BAM2019::computeSublaneShift()" <<std::endl;
    int prevShift = 0;
...
int
MSLCM_BAM2019::_wantsChangeSublane(
    int laneOffset,
    LaneChangeAction alternatives,
    const MSLeaderDistanceInfo& leaders,
    const MSLeaderDistanceInfo& followers,
    ...
    MSVehicle** firstBlocked,
    double& latDist, double& maneuverDist, int& blocked) {
    std::cout <<"-------> MSLCM_BAM2019::_wantsChangeSublane Called! " <<std::endl;
  ...
  /*  ret = slowDownForBlocked(lastBlocked, ret);
    ...
    if (lastBlocked != firstBlocked) {
         std::cout <<"$$$$$$$$$$$ slowDownForBlocked = " << std::endl;
    ...
    */
    ...
    if ((ret & LCA_URGENT) != 0) {
        ...
        std::cout <<"$$$$$$$$$$$ prepare urgent lane >>  ret & LCA_URGENT = " << std::endl;
    ...
    if (roundaboutEdgesAhead > 1) {
        ...
        std::cout <<"$$$$$$$$$$$ roundaboutEdgesAhead = " << std::endl;
        if (left) {
            ret |= LCA_COOPERATIVE;
        } else {
    ...
    if (laneOffset != 0
        ...
        if (!cancelRequest(ret)) {
            std::cout <<"$$$$$$$$$$$ amBlockingFollowerPlusNB = "<< std::endl;
    ...
    if (!left) {
         std::cout <<"$$$$$$$$$$$ ONLY FOR CHANGING TO THE RIGHT = " << std::endl;
    ...
    if (!right) {
      std::cout <<"$$$$$$$$$$$ ONLY FOR CHANGING TO THE LEFT = "<< std::endl;
...
int
MSLCM_BAM2019::slowDownForBlocked(MSVehicle** blocked, int state) {
...
void
MSLCM_BAM2019::saveBlockerLength(const MSVehicle* blocker, int lcaCounter) {
...
void MSLCM_BAM2019::addLCSpeedAdvice(const double vSafe) {
...
void
MSLCM_BAM2019::updateExpectedSublaneSpeeds(const MSLeaderDistanceInfo& ahead, int sublaneOffset, int laneIndex) {
...
double
MSLCM_BAM2019::computeSpeedGain(double latDistSublane, double defaultNextSpeed) const {
    ...
    const double leftVehSide = rightVehSide + vehWidth;
    std::cout << "overlap() Called in MSLCM_BAM2019::computeSpeedGain() "<< std::endl;
...
CLeaderDist
MSLCM_BAM2019::getLongest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
    double maxLength = -1;
...
CLeaderDist
MSLCM_BAM2019::getSlowest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
...
int
MSLCM_BAM2019::checkBlocking(const MSLane& neighLane, double& latDist, double& maneuverDist, int laneOffset,
                            const MSLeaderDistanceInfo& leaders,
                            const MSLeaderDistanceInfo& followers,
                            const MSLeaderDistanceInfo& /*blockers */,
                            const MSLeaderDistanceInfo& neighLeaders,
...
int
MSLCM_BAM2019::checkBlockingVehicles(
    ...
    for (int i = 0; i < vehicles.numSublanes(); ++i) {
        ...
        if (vehDist.first != 0 && myCFRelated.count(vehDist.first) == 0) {
            ...
            std::cout <<"---------> overlap()  MSLCM_BAM2019::checkBlockingVehicles" <<std::endl;
...
void
MSLCM_BAM2019::updateCFRelated(const MSLeaderDistanceInfo& vehicles, double foeOffset, bool leaders) {
    ...
    for (int i = 0; i < vehicles.numSublanes(); ++i) {
        ...
        if (vehDist.first != 0 && myCFRelated.count(vehDist.first) == 0) {
            double foeRight, foeLeft;
            vehicles.getSublaneBorders(i, foeOffset, foeRight, foeLeft);
            std::cout <<"-------->  overlap() called in MSLCM_BAM2019::updateCFRelated ()" <<std::endl;
...
bool
MSLCM_BAM2019::overlap(double right, double left, double right2, double left2) {
    std::cout <<"*****^^^^ overlap right = "<<right <<"; left  = " << left << " ; right2 = " <<right2 <<" and left2 = "<<left2 <<std::endl;
/*    assert(right <= left);
    assert(right2 <= left2);
...
MSLCM_BAM2019::StateAndDist
MSLCM_BAM2019::decideDirection(StateAndDist sd1, StateAndDist sd2) const {
...
LaneChangeAction
MSLCM_BAM2019::getLCA(int state, double latDist) {
...
int
MSLCM_BAM2019::checkStrategicChange(int ret,
                                   int laneOffset,
                                   const std::vector<MSVehicle::LaneQ>& preb,
                                   const MSLeaderDistanceInfo& leaders,
                                   const MSLeaderDistanceInfo& neighLeaders,
...
double
MSLCM_BAM2019::computeGapFactor(int state) const {
    return (state & LCA_STRATEGIC) != 0 ? MAX2(0.0, (1.0 - myPushy * (1 + 0.5 * myImpatience))) : 1.0;
}


int
MSLCM_BAM2019::keepLatGap(int state,
                         const MSLeaderDistanceInfo& leaders,
                         const MSLeaderDistanceInfo& followers,
                         const MSLeaderDistanceInfo& blockers,
                         const MSLeaderDistanceInfo& neighLeaders,
...
void
MSLCM_BAM2019::updateGaps(const MSLeaderDistanceInfo& others, double foeOffset, double oldCenter, double gapFactor,
...
double
MSLCM_BAM2019::getWidth() const {
    return myVehicle.getVehicleType().getWidth() + NUMERICAL_EPS;
}


double
MSLCM_BAM2019::computeSpeedLat(double latDist, double& maneuverDist) {
...
void
MSLCM_BAM2019::commitManoeuvre(int blocked, int blockedFully,
                              const MSLeaderDistanceInfo& leaders,
                              const MSLeaderDistanceInfo& neighLeaders,
                              const MSLane& neighLane,
                              double maneuverDist) {
...
double
MSLCM_BAM2019::commitFollowSpeed(double speed, double latDist, double secondsToLeaveLane, const MSLeaderDistanceInfo& leaders, double foeOffset) const {
...
#ifdef DEBUG_MANEUVER
                if (gDebugFlag2) {
                    std::cout <<" .......> overlap() checked by gDebugFlag2 in MSLCM_BAM2019::commitFollowSpeed()" <<std::endl;
                    std::cout << "   foe=" << vehDist.first->getID()
                              << " gap=" << vehDist.second
...
#endif
                std::cout <<"--------> overlap() checked in MSLCM_BAM2019::commitFollowSpeed()" <<std::endl;
...
std::string
MSLCM_BAM2019::getParameter(const std::string& key) const {
...
void
MSLCM_BAM2019::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
...
#endif
    std::cout <<" -----? > MSLCM_BAM2019::wantsChange() " <<std::endl;
    double latDist = 0;
