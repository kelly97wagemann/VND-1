 ...
 * static members
 * ----------------------------------------------------------------------- */
bool MSAbstractLaneChangeModel::myAllowOvertakingRight(true);  //BAM modification
// bool MSAbstractLaneChangeModel::myAllowOvertakingRight(false); //Original
...
MSAbstractLaneChangeModel::build(LaneChangeModel lcm, MSVehicle& v) {
    std::cout <<"Chosen Lane Change model = " <<lcm <<std::endl;
    if (MSGlobals::gLateralResolution > 0 && lcm != LCM_SL2015 && lcm != LCM_BAM2019  && lcm != LCM_DEFAULT) {
        throw ProcessError("Lane change model '" + toString(lcm) + "' is not compatible with sublane simulation");
    }
 
   switch (lcm) {
        case LCM_DK2008:
        std::cout <<"*********>   Lane Change is : LCM_DK2008 " <<LCM_DK2008 << std::endl;
            return new MSLCM_DK2008(v);
        case LCM_LC2013:
            std::cout <<"*********>   Lane Change is : MSLCM_LC2013 " << std::endl;
            return new MSLCM_LC2013(v);
        case LCM_SL2015:
            std::cout <<"*********>   Lane Change is : MSLCM_SL2015 " << std::endl;
            return new MSLCM_SL2015(v);
        case LCM_BAM2019:
            std::cout <<"*********>   Lane Change is : MSLCM_BAM2019 "  << std::endl;
            return new MSLCM_BAM2019(v);
        case LCM_DEFAULT:
...
MSAbstractLaneChangeModel::startLaneChangeManeuver(MSLane* source, MSLane* target, int direction) {
    std::cout <<" BAM MSAbstractLaneChangeModel::startLaneChangeManeuver() initiated " <<std::endl;
