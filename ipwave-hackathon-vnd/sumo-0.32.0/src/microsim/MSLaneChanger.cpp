...
#define OPPOSITE_OVERTAKING_ONCOMING_LOOKAHEAD 200.0 // just a guess*/ //BAM Comment, Next are modified Parameters

#define OPPOSITE_OVERTAKING_MAX_LOOKAHEAD 150.0 // just a guess
#define OPPOSITE_OVERTAKING_MAX_LOOKAHEAD_EMERGENCY 150.0 // just a guess
// this is used for finding oncoming vehicles while driving in the opposite direction
#define OPPOSITE_OVERTAKING_ONCOMING_LOOKAHEAD 150.0 // just a guess
...
MSLaneChanger::laneChange(SUMOTime t) {
    ...
    std::cout <<"MSLaneChanger::laneChange(SUMOTime t), Time = " <<t <<std::endl;
    initChanger();
    try {
        while (vehInChanger()) {
...
MSLaneChanger::change() {
    ...
    // priority.
    std::cout <<"<<--->> change() of  MSLaneChanger.cpp" <<std::endl;
#ifdef DEBUG_ACTIONSTEPS
//        std::cout<< "\nCHANGE" << std::endl;
#endif
    std::cout <<"====MSLaneChanger::change() " <<std::endl;
...
MSLaneChanger::checkChange(
    ...
    std::cout <<"MSLaneChanger, checkChange() called @!! " <<std::endl;
...
MSLaneChanger::changeOpposite(std::pair<MSVehicle*, double> leader) {
    ...
    if ((state & LCA_WANTS_LANECHANGE) != 0 && changingAllowed
        ...
        vehicle->getLaneChangeModel().startLaneChangeManeuver(source, opposite, direction); //Stopped by BAM
