...
MSLaneChangerSublane::change() {
    ...
    std::cout <<" ========>>>>>> MSLaneChangerSublane::change() " <<std::endl;
...
assert(vehicle->getLane() == (*myCandi).lane);
    ...
    if (!vehicle->isActive()) {
        ...
        std::cout <<" ------ >>>>>   TraCI causes Lane Change!  "<<std::endl;
#endif
   ...
   if (vehicle->getLaneChangeModel().debugVehicle()) {
    ...
    vehicle->getLaneChangeModel().setManeuverDist(decision.maneuverDist); //BAM Comment
...
MSLaneChangerSublane::continueChangeSublane(MSVehicle* vehicle, ChangerIt& from) {
    std::cout << SIMTIME << " MSLaneChangerSublane::continueChangeSublane --- > vehicle '" << vehicle->getID() << std::endl;
...
MSLaneChangerSublane::checkChangeSublane(
    ...
    std::cout <<" ------ MSLaneChangerSublane::checkChangeSublane Called!! " <<std::endl;
